...
# LSTM 모델 정의
model = Sequential()
model.add(LSTM(144, return_sequences=True, input_shape=(144, 1)))
model.add(LSTM(64, return_sequences=False))
model.add(Dense(future_period, activation='linear'))
model.compile(loss='mse', optimizer='adam')

# 모델 훈련
model.fit(x_train_reshape, y_train, validation_data=(x_test_reshape, y_test), batch_size=10, epochs=30)

# 모델 저장
model.save('qkrdata10.model')
